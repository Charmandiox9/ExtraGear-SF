...
import io.github.thebusybiscuit.extragear.util.CustomItemStack;
...
public class ExtraGear extends JavaPlugin implements SlimefunAddon {
    ...
    public void onEnable() {
        ...
        if (cfg.getBoolean("options.auto-update") && getDescription().getVersion().startsWith("DEV - ")) {
            try {
                ...
                getLogger().log(Level.WARNING, "Could not check for updates: {0}", e.getMessage());
            }
        }
    ...
    private void registerSword(Material type, String component, SlimefunItemStack item, List<Pair<Enchantment, Integer>> enchantments) {
        ...
        SlimefunItem slimefunItem = new SlimefunItem(itemGroup, is, RecipeType.ENHANCED_CRAFTING_TABLE, new ItemStack[] {
            null, item.item(), null,
            null, item.item(), null,
    ...
    private void registerArmor(ArmorSet armorset, String component, SlimefunItemStack item, List<Pair<Enchantment, Integer>> enchantments) {
        ...
        SlimefunItemStack[] armor = {
            new SlimefunItemStack(component + "_HELMET", armorset.getHelmet(), "&f" + humanizedComponent + " Helmet"),
